...
class modbusMaster {
 ...
 private:
    ...
    Stream*
        _stream;  // The stream instance (serial port) for communication with the RS485
    int8_t _enablePin;  // The pin controlling the driver/receiver enable on the
                        // RS485-to-TLL chip
    ...
    // of silence between characters in a frame and any space over 3.5 characters
    // defines a new frame.
    const uint32_t modbusTimeout =
        MODBUS_TIMEOUT;  // The time to wait for response after a command (in ms)
    const int modbusFrameTimeout =
        MODBUS_FRAME_TIMEOUT;  // the time to wait between characters within a frame (in
                               // ms)
};
